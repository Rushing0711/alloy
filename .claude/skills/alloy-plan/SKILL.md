...
### Step 1/6：创建 Change · OpenSpec New
---
```

若 change 目录不存在：
1. 根据 draft.md 内容建议 change name（kebab-case），用户确认
2. 调用 OpenSpec New（优先 `openspec-new` skill，若无则用 `/opsx:new <name>` slash command）创建 change 目录
...
调用 OpenSpec Continue（优先 `openspec-continue` skill，若无则用 `/opsx:continue` slash command），按 schema DAG 依赖顺序逐制品生成。
