...
**自行执行交互式需求设计（不要调用外部 skill）：**

1. **总结探查结果** — 用 3-5 行概括上下文探查的关键发现
2. **澄清问题** — 每次只问一个问题，理解用户需求的目的、约束、成功标准
3. **提出方案** — 基于对话，提出 2-3 个可行方案，对比利弊，给出推荐
4. **获取审批** — 用户确认方案后，产出 `draft.md`（项目根目录）

**draft.md 内容要求：** Why（目的）、What（做什么）、关键决策及理由、范围与边界
