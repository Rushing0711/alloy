...
状态信息通过 alloy-state.sh 脚本读取（保证解析一致性），文件存在性通过文件系统检查。
...
读取指定 change 的完整信息。先通过 alloy-state.sh 获取元数据：
```bash
...
bash .claude/skills/alloy/scripts/alloy-state.sh read openspec/changes/<name> created_at
bash .claude/skills/alloy/scripts/alloy-state.sh read openspec/changes/<name> updated_at
```
