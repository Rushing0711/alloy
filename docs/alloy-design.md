...
无活跃 change + 有 topic:
  ...
  → 分支选择：确认主分支（写入 openspec/config.yaml）→ 检测当前分支，
    若在主分支则 HARD STOP（不允许在主分支开发）→ 展示选项（切换到已有非主分支 / 新建 feature 分支），
    新建默认命名 feature/<change-name>，写入 .alloy.yaml 的 feature_branch 字段
...
执行: superpowers:finishing-a-development-branch
  ...
  → 3 选项:
      1. 本地 merge → 合入 main_branch → phase → finished，完成
...
3. 分流：
   ...
   不改 spec（实现偏离现有 spec）:
     → TDD 修复 → verification-before-completion → 直接提交，或创建 PR 时进行 code review

   需改 spec（spec 需新增或修正）:
     ├── 有活跃 change 且 phase < applied（无代码落地）
     │     → "spec 变更可并入当前 change <name>。运行 /alloy:start <name> 重新 brainstorming。"
     │     → 无需开新 change
     │
     ├── 有活跃 change 且 phase ≥ applied（已有代码落地）
     │     → 推荐"先完成当前 change"（测试→archive→finish），后续开新 change 修 spec + 修 bug
     │     → 可选 discard（代码有严重问题时）或记下建议名称手动决定
     │     → 不自动创建新 change
     │
     └── 无活跃 change
...
详见 [alloy-visual-spec.md](alloy-visual-spec.md)——从本文档提取的独立视觉规范，以实际落地格式为准。写 Skill 时参考该文档。
