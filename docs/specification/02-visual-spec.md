...
**规则：**
- 本规范描述 Agent 的**终端输出**格式。Skill 文件内部使用 `### [Step N/M] 描述` 作为 Markdown 标题组织文档结构，两者不冲突——Agent 输出到终端时使用纯文本格式
...
| 制品 | 编号 | 下一个 | 依赖 |
|------|------|--------|------|
| proposal | [1/5] | design | draft.md |
| design | [2/5] | specs | proposal + draft.md |
| specs | [3/5] | tasks | proposal |
| tasks | [4/5] | 生成 plans | specs + design |
| plans | [5/5] | plan 阶段完成 | tasks |
...
| 制品 | 编号 | 下一个 | 依赖 |
|------|------|--------|------|
| verify | [1/2] | retrospective | plans |
| retrospective | [2/2] | apply 阶段完成 | verify |
...
draft 不走审查窗口——审批嵌入 brainstorming 交互式对话流程，用户确认方案后直接生成 draft.md。
...
各阶段完成框之后展示 `→` 属性行 + 制品汇总表（§八）+ 下一步引导。

**审查窗口对照：** Plan 和 Apply 阶段的制品在审批时各自走了 §三 的审查窗口（Plan 5 个 + Apply 2 个），完成后在此展示汇总表。draft 不走审查窗口，只在此展示。
