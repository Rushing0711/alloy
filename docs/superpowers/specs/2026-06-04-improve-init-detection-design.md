...
创建 `src/core/detect-installations.ts`，提供统一的检测函数供所有安装模块调用。
...
function detectCommand(name: string, projectPath: string): InstallationInfo;
function detectSkill(name: string, projectPath: string): InstallationInfo;
```
...
│
├─ 3. 初始化 OpenSpec 项目（新增检测）
│     detectCommand("opsx/continue") + detectSkill("openspec-explore")
│     ├─ 都不存在 → 正常 openspec init
│     └─ 存在 → "OpenSpec 已初始化（commands: ✓/✗, skills: ✓/✗）"
│                "openspec init 可能覆盖，继续？" → 用户选择
│
├─ 4. 安装 Superpowers（新增检测）
│     detectSkill("brainstorming")
│     ├─ not found → 正常安装
│     └─ found → 版本比较 → 提示覆盖/跳过/升级 → 用户选择
│
├─ 5. 部署 Alloy commands（新增检测）
│     detectCommand("alloy/start")
│     ├─ not found → 正常部署
│     └─ found → "已部署（位置），是否覆盖？" → 用户选择
│
