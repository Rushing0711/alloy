...
[AQ] radio: (a) 确认变更，回到 brainstorming / (b) 取消，继续 apply。降级：`> 请输入 a 或 b：`
...
- **feature_branch 为 null** → ⚠️ 状态异常，需修复：
  [AQ] radio: (a) 手动指定分支名 / (b) 使用当前分支 `<current_branch>` / (c) 放弃，回退 start。降级：`> 请输入 a、b 或 c：`
  用户选择后，`alloy _state write openspec/changes/<name> feature_branch <用户确认的分支>`，然后继续。**null 是状态异常，不是设计的容错——不允许静默回退。**
- **当前分支 ≠ feature_branch** → 提示切换：[AQ] radio: (a) 切换到 <feature_branch>（推荐）/ (b) 使用当前分支（更新记录）。降级：`> 请输入 a 或 b：`
  选 (a)：`git checkout <feature_branch>` → 继续
  选 (b)：`alloy _state write openspec/changes/<name> feature_branch <current_branch>` → 继续
- **feature_branch 不存在本地** → ⚠️ 分支丢失：[AQ] radio: (a) 从远程拉取 / (b) 手动指定其他分支 / (c) 放弃，回退 start。降级：`> 请输入 a、b 或 c：`
...
3. 展示推荐方案（来自 plans.md header），用户可覆写：[AQ] radio: subagent-driven-development（推荐）/ executing-plans。降级：`> 请输入 1 或 2：`
...
>
> → 下一个：retrospective
> [AQ] 审查确认: (a) 确认，锁定 verify 并继续 retrospective / (b) 需要调整 — 说明修改点。降级：`> 请输入 a 或 b：`
...
>
> → 下一个：完成 apply 阶段
> [AQ] 审查确认: (a) 确认，锁定 retrospective 并完成 apply / (b) 需要调整 — 说明修改点。降级：`> 请输入 a 或 b：`
