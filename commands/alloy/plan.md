...
- **选 (b)**：制品未通过审查 → 停止 plan 流程。任何需求/设计层面的调整都应从 draft 根重新审视，引导用户运行 `/alloy:start --new <topic>` 重新走 brainstorming 流程。typo/措辞等纯文本修正除外——可直接修改后重新展示
...
| 要修改的制品 | 行为 |
|------------|------|
| draft | 全部过期 → 引导 `/alloy:start --new <topic>` |
| proposal | 全部过期 → 引导 `/alloy:start --new <topic>` |
| design | 全部过期 → 引导 `/alloy:start --new <topic>` |
| specs | 全部过期 → 引导 `/alloy:start --new <topic>` |
| tasks | 全部过期 → 引导 `/alloy:start --new <topic>` |
| plans | 全部过期 → 引导 `/alloy:start --new <topic>` |
...
→ 请运行 /alloy:start --new <topic> 重新走需求确认流程
```
